...
class RSD(DDIM):
    def __init__(self, model: SDInversePipeline, cfg: DictConfig):
        ...
        self.sigma_x0 = cfg.algo.get("sigma_x0", 0.0)
        self.gamma = cfg.algo.get("gamma", 0.0)
        self.w_t = cfg.algo.get("w_t", 0.15)
        self.obs_weight = cfg.algo.get("obs_weight", 1.0)
        self.lr_x = cfg.algo.get("lr_x", 0.2)
        self.rho_reg = cfg.algo.get("rho_reg", 0.2)
        self.dino_flag = cfg.algo.get("dino_flag", False)
        self.sigma_break = cfg.algo.get("sigma_break", 600)

        self.height = 512
        self.width = 512
        self.num_channels_latents = cfg.model.C
        self.device = self.model._execution_device
        self.all_sigmas = torch.sqrt(
        ...
        if self.load_img:
            self.evol_path = f"_exp/evol/{cfg.algo.name}_single_{self.cfg.algo.deg}_{self.cfg.exp.img_id}_{self.gamma}_{self.lr_z}"
        else:
            self.evol_path = f"_exp/evol/{cfg.algo.name}"

        if not os.path.exists(f"{self.evol_path}"):
            os.makedirs(f"{self.evol_path}")
        else:
            shutil.rmtree(f"{self.evol_path}")
            os.makedirs(f"{self.evol_path}")
