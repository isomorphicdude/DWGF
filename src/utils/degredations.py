...
class Inpainting(H_functions):
    def __init__(self, channels, img_dim, missing_indices, device):
        ...
        num_pixels = channels * img_dim**2
        mask = torch.ones(num_pixels, dtype=torch.bool, device=device)
        mask[missing_indices] = False
        self.kept_indices = mask.nonzero(as_tuple=False).squeeze(1)
        # self.kept_indices = (
        #     torch.Tensor(
        ...
        #     .long()
        # )
        print("kept_indices.shape", self.kept_indices.shape)
...
def build_one_degredation_model(cfg, h, w, c, deg: str):
    ...
    if deg == "deno":
        ...
        if deg == "inp_lolcat":
            ...
            aux = (
                (torch.arange(300 + 512 * 200, 400 + 512 * 200).cuda().long())
                .cuda()
                .long()
            )
            ...
            if w == 512:
                aux = (
                    (torch.arange(250 + 512 * 150, 450 + 512 * 150).cuda().long())
                    .cuda()
                    .long()
                )
                ...
                aux = (
                    (torch.arange(125 + 256 * 75, 225 + 256 * 75).cuda().long())
                    .cuda()
                    .long()
                )
            ...
            if w == 512:
                aux = (
                    (torch.arange(150 + 512 * 150, 375 + 512 * 150).cuda().long())
                    .cuda()
                    .long()
                )
    ...
    #     H = Inpainting(c, w, m, device)
        ...
        H = NonlinearBlurOperator(
            device=device, opt_yml_path=opt_yml_path, current_dir=current_dir, w=w
        )
